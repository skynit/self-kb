...
async function processWithClaude(app, transcript, generateSummary, settings, statusCallback, RetryModal, transcriptLanguageCode) {
  ...
  const makeRequest = async () => {
    ...
    const data = response.json;
    const rawContent = (_g = (_f = data.content) == null ? void 0 : _f[0]) == null ? void 0 : _g.text;
    if (!rawContent) {
      if (statusCallback) statusCallback(null);
...
var FileSuggest = class extends PathSuggest {
  getSuggestionsInternal(input) {
    const abstractFiles = this.app.vault.getAllLoadedFiles();
    const files = [];
    const lowerCaseInput = input.toLowerCase();
    for (const file of abstractFiles) {
      if (file instanceof import_obsidian17.TFile && file.extension === "md" && file.path.toLowerCase().includes(lowerCaseInput)) {
        files.push(file);
      }
    }
    return files;
...
var YouTubeTranscriptSettingTab = class extends import_obsidian19.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian19.Setting(containerEl).setName("YouTube transcript").setHeading();
    void this.refreshModelLists();
    const version = this.plugin.manifest.version;
    new import_obsidian19.Setting(containerEl).setName("Version").setDesc(`Plugin version: ${version}`).setDisabled(true);
    new import_obsidian19.Setting(containerEl).setName("Files and folders").setHeading();
    new import_obsidian19.Setting(containerEl).setName("Available file formats").setDesc(
    ...
    const formats = ["markdown", "pdf", "srt"];
    const formatNames = {
      markdown: "Markdown (.md)",
      pdf: "PDF",
      srt: "SRT Subtitles (.srt)"
    ...
    formats.forEach((format) => {
      ...
      checkbox.checked = this.settings.fileFormats && this.settings.fileFormats.includes(format);
      checkbox.addEventListener("change", () => {
    ...
    new import_obsidian19.Setting(containerEl).setName("Transcript directories").setHeading();
    const savedDirs = this.settings.savedDirectories || [];
    if (savedDirs.length > 0) {
      new import_obsidian19.Setting(containerEl).setName("Default directory").setDesc(
        ...
        dropdown.setValue(this.settings.defaultDirectory || "").onChange(async (value) => {
    ...
    new import_obsidian19.Setting(containerEl).setName("Manage directories").setDesc(
    ...
    const renderDirectoriesList = () => {
      ...
      const savedDirs2 = this.settings.savedDirectories || [];
      const defaultDir = this.settings.defaultDirectory;
      if (savedDirs2.length === 0) {
        directoriesList.createEl("p", {
          text: "No directories saved. Add one below.",
          attr: { style: "color: var(--text-muted); font-style: italic;" }
        });
      } else {
        savedDirs2.forEach((dir, index) => {
          ...
          if (defaultDir === dir) {
            dirItem.createSpan({
              text: "(Default)",
              attr: {
                style: "color: var(--text-accent); font-size: 0.9em; font-weight: 500;"
              }
            });
          }
          ...
          removeButton.onclick = async () => {
            ...
            this.settings.savedDirectories = savedDirs2.filter(
              (_, i) => i !== index
            );
            await this.saveSettings();
            this.display();
    ...
    addButton.onclick = async () => {
      ...
      if (newDir && newDir !== "") {
        const normalizedDir = newDir.replace(/^\/+|\/+$/g, "").replace(/\\/g, "/");
        const savedDirs2 = this.settings.savedDirectories || [];
        if (!savedDirs2.includes(normalizedDir)) {
          this.settings.savedDirectories = [...savedDirs2, normalizedDir];
          await this.saveSettings();
    ...
    new import_obsidian19.Setting(containerEl).setName("Transcript").setHeading();
    new import_obsidian19.Setting(containerEl).setName("Create new markdown file").setDesc(
      ...
      var _a;
      toggle.setValue((_a = this.settings.createNewFile) != null ? _a : false).onChange(async (value) => {
        this.settings.createNewFile = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Default note name").setDesc(
      "Template for note file names. Supports {VideoName} and {ChannelName} variables."
    ).addText((text) => {
      text.setPlaceholder("{VideoName}").setValue(this.settings.defaultNoteName || "{VideoName}").onChange(async (value) => {
        this.settings.defaultNoteName = value || "{VideoName}";
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Preferred languages").setDesc(
      ...
      text.setPlaceholder("en,es,fr").setValue(this.settings.preferredLanguage || "").onChange(async (value) => {
        const normalizedValue = value.split(",").map((lang) => lang.trim().toLowerCase()).filter((lang) => lang.length > 0).join(",");
        this.settings.preferredLanguage = normalizedValue;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Include video URL").setDesc(
      "When enabled, the video URL will be included in the transcript (can be overridden in the modal)"
    ).addToggle((toggle) => {
      toggle.setValue(this.settings.includeVideoUrl).onChange(async (value) => {
        this.settings.includeVideoUrl = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Generate summary").setDesc(
    ...
    new import_obsidian19.Setting(containerEl).setName("Tag with channel name").setDesc(
      "When enabled, notes will be tagged with the YouTube channel name (can be overridden in the modal)"
    ).addToggle((toggle) => {
      toggle.setValue(this.settings.tagWithChannelName).onChange(async (value) => {
        this.settings.tagWithChannelName = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Single line transcript").setDesc(
      ...
      var _a;
      toggle.setValue((_a = this.settings.singleLineTranscript) != null ? _a : false).onChange(async (value) => {
        this.settings.singleLineTranscript = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Allow clipboard access").setDesc(
      ...
      var _a;
      toggle.setValue((_a = this.settings.allowClipboardAccess) != null ? _a : true).onChange(async (value) => {
        this.settings.allowClipboardAccess = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Prevent duplicate notes").setDesc(
      ...
      var _a;
      toggle.setValue((_a = this.settings.checkForDuplicates) != null ? _a : false).onChange(async (value) => {
        this.settings.checkForDuplicates = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Duplicate check property").setDesc(
      'The frontmatter property used to detect duplicates. Defaults to "url", which the plugin writes automatically.'
    ).addText((text) => {
      text.setPlaceholder("url").setValue(this.settings.duplicateCheckProperty || "url").onChange(async (value) => {
        this.settings.duplicateCheckProperty = value.trim() || "url";
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Timestamp").setHeading();
    new import_obsidian19.Setting(containerEl).setName("Include timestamps").setDesc(
      "When enabled, timestamps will be included in transcripts as clickable links to the video at that time"
    ).addToggle((toggle) => {
      toggle.setValue(this.settings.includeTimestamps).onChange(async (value) => {
        this.settings.includeTimestamps = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Timestamp frequency").setDesc(
      "How often to show timestamps: 0 = every sentence, >0 = every N seconds (e.g., 30 = every 30 seconds)"
    ).addText((text) => {
      var _a;
      text.inputEl.type = "number";
      text.setPlaceholder("0").setValue(((_a = this.settings.timestampFrequency) == null ? void 0 : _a.toString()) || "0").onChange(async (value) => {
    ...
    new import_obsidian19.Setting(containerEl).setName("Include timestamps in LLM output").setDesc(
      ...
      toggle.setValue(this.settings.includeTimestampsInLLM).onChange(async (value) => {
        this.settings.includeTimestampsInLLM = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Local video directory").setDesc(
      ...
      text.setPlaceholder("/path/to/videos").setValue(this.settings.localVideoDirectory || "").onChange(async (value) => {
        const normalizedPath = value.trim().replace(/\\/g, "/").replace(/\/+$/, "");
        this.settings.localVideoDirectory = normalizedPath;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("PDF").setHeading();
    new import_obsidian19.Setting(containerEl).setName("Create cover note").setDesc("When enabled, a cover note will be created for PDF and/or SRT files").addToggle((toggle) => {
      var _a;
      toggle.setValue((_a = this.settings.createCoverNote) != null ? _a : false).onChange(async (value) => {
        this.settings.createCoverNote = value;
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Cover note location").setDesc(
    ...
    new import_obsidian19.Setting(containerEl).setName("Attachment folder").setDesc(
      ...
      text.setPlaceholder("attachments").setValue(this.settings.attachmentFolder || "").onChange(async (value) => {
        this.settings.attachmentFolder = value.trim().replace(/[/\\]+/g, "").trim();
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Default SRT file name").setDesc(
      "Template for SRT file names. Supports {VideoName} and {ChannelName} variables."
    ).addText((text) => {
      text.setPlaceholder("{VideoName}").setValue(this.settings.defaultSrtFileName || "{VideoName}").onChange(async (value) => {
        this.settings.defaultSrtFileName = value || "{VideoName}";
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("Cover note template").setDesc(
      ...
      text.setPlaceholder("Templates/Cover Note.md").setValue(this.settings.coverNoteTemplate || "").onChange(async (value) => {
        const normalizedPath = value.trim().replace(/^\/+|\/+$/g, "").replace(/\\/g, "/");
    ...
    new import_obsidian19.Setting(containerEl).setName("Cover note file name").setDesc(
      "Template for cover note file names. Supports {VideoName} and {ChannelName}. Default: {VideoName}"
    ).addText((text) => {
      text.setPlaceholder("{VideoName}").setValue(this.settings.defaultCoverNoteName || "").onChange(async (value) => {
        this.settings.defaultCoverNoteName = value.trim();
        await this.saveSettings();
      });
    });
    new import_obsidian19.Setting(containerEl).setName("LLM").setHeading();
    new import_obsidian19.Setting(containerEl).setName("Use LLM processing").setDesc("When enabled, transcripts will be processed by the selected LLM provider to clean up and format the content").addToggle(
      (toggle) => {
        var _a;
        return toggle.setValue((_a = this.settings.useLLMProcessing) != null ? _a : false).onChange(async (value) => {
    ...
    if (this.settings.useLLMProcessing) {
      new import_obsidian19.Setting(containerEl).setName("LLM provider").setDesc("Select which LLM provider to use for transcript processing").addDropdown((dropdown) => {
        dropdown.addOption("openai", "OpenAI").addOption("gemini", "Google Gemini").addOption("claude", "Anthropic Claude");
        ...
        dropdown.setValue(this.settings.llmProvider || "openai").onChange(async (value) => {
      ...
      if (this.settings.llmProvider === "openai") {
        new import_obsidian19.Setting(containerEl).setName("OpenAI API key").setDesc(
      ...
      if (this.settings.llmProvider === "gemini") {
        new import_obsidian19.Setting(containerEl).setName("Gemini API key").setDesc(
      ...
      if (this.settings.llmProvider === "claude") {
        new import_obsidian19.Setting(containerEl).setName("Claude API key").setDesc(
        ...
        new import_obsidian19.Setting(containerEl).setName("Claude model").setDesc(
          ...
          text.setPlaceholder("claude-sonnet-4-20250514").setValue(this.settings.claudeModel || DEFAULT_SETTINGS.claudeModel).onChange(async (value) => {
      ...
      const promptSetting = new import_obsidian19.Setting(containerEl).setName("Processing prompt").setDesc("The prompt sent to the LLM for processing the transcript");
      ...
      new import_obsidian19.Setting(containerEl).setName("LLM timeout").setDesc(
        "Timeout for LLM API requests in minutes (default: 1 minute / 60 seconds)"
      ).addText((text) => {
        text.inputEl.type = "number";
        text.setPlaceholder("5").setValue(this.settings.openaiTimeout.toString()).onChange(async (value) => {
      ...
      new import_obsidian19.Setting(containerEl).setName("Force LLM output language").setDesc(
        ...
        var _a;
        toggle.setValue((_a = this.settings.forceLLMLanguage) != null ? _a : false).onChange(async (value) => {
          this.settings.forceLLMLanguage = value;
          await this.saveSettings();
        });
      });
      new import_obsidian19.Setting(containerEl).setName("Custom LLM Providers").setDesc(
      ...
      new import_obsidian19.Setting(containerEl).addButton((button) => {
        button.setButtonText("Add custom provider").setCta().onClick(() => {
   ...
   */
  createOpenAIModelSetting(containerEl) {
    const setting = new import_obsidian19.Setting(containerEl).setName("OpenAI model").setDesc("Select the OpenAI model to use for transcript processing");
   ...
   */
  createGeminiModelSetting(containerEl) {
    const setting = new import_obsidian19.Setting(containerEl).setName("Gemini model").setDesc("Select the Gemini model to use for transcript processing");
  ...
  renderCustomProvidersList(containerEl) {
    const customProviders = this.settings.customProviders || [];
    ...
    if (customProviders.length === 0) {
      ...
      customProviders.forEach((provider) => {
        const providerItem = providersList.createDiv({
          attr: {
            style: "display: flex; align-items: center; gap: 0.5em; margin-bottom: 0.5em; padding: 0.5em; background: var(--background-secondary); border-radius: 4px;"
          }
        });
        ...
        infoDiv.createSpan({
          ...
          attr: {
            style: "font-family: monospace; font-size: 0.9em; color: var(--text-muted);"
          }
        });
...
var YouTubeTranscriptPlugin = class extends import_obsidian21.Plugin {
  ...
  async processTranscript(options) {
    ...
    try {
      ...
      if (createNewFile) {
        ...
        if (fileFormat === "pdf") {
          new import_obsidian21.Notice("PDF format requires creating a new file. Please enable 'Create new file' in settings or use the modal to create a PDF.", 1e4);
          return;
        }
        if (fileFormat === "srt") {
          new import_obsidian21.Notice("SRT format requires creating a new file. Please enable 'Create new file' in settings or use the modal to create an SRT file.", 1e4);
          return;
        }
  ...
  async createTranscriptFile(options) {
    ...
    if (videoDetails) {
      ...
      if (videoDetails.isLiveContent !== void 0) frontmatter.isLive = videoDetails.isLiveContent;
      if (videoDetails.isPrivate !== void 0) frontmatter.isPrivate = videoDetails.isPrivate;
      if (videoDetails.isUnlisted !== void 0) frontmatter.isUnlisted = videoDetails.isUnlisted;
