...
![[70-项目/01-CWNAS/01-设计文档/附件/Sync1-客户端与NAS.png|320]]

#### NAS 与网盘

![[70-项目/01-CWNAS/01-设计文档/附件/Sync1-NAS与网盘.png|320]]
...
![[70-项目/01-CWNAS/01-设计文档/附件/Sync1-任务执行流程.png|760]]
...
**电脑与 NAS 同步，由客户端调用 rclone；
NAS 与网盘同步，由服务端调用 rclone。**

两端后台服务持续运行，rclone 按任务启动子进程，完成后退出。下列命令表示参数结构，目录和远端名称均由任务配置生成。

##### 4.2.1 客户端：执行电脑与 NAS 同步

**运行位置：用户电脑上的 CWNAS 客户端后台服务。** 即使选择“NAS → 电脑”，也是客户端主动连接 NAS 并下载文件。如果完成了websocket的消息通知模块，可以用消息推送的形式，推送服务端文件更新。
...
![[70-项目/01-CWNAS/01-设计文档/附件/Sync1-穿透数据流转.png|320]]
